#ifndef FTK_INDEXEDMINHEAP_HPP
#define FTK_INDEXEDMINHEAP_HPP
...
class MultiLevel{
    public:
        ...
        inline bool aggiorna( MultiLevel &other){
            bool all_over = true;
        ...
        inline bool operator<(const MultiLevel& other) const {
...
class IndexedMinHeap{
    public:
        ...
        inline IndexedMinHeap(){}
        inline IndexedMinHeap( size_t tot_object, size_t max_k) : max_k(max_k){
            indexed_map.resize(tot_object);
            for (size_t i = 0; i < tot_object; i++){
                indexed_map[i].second = -1;
            }
        }
        inline void inizializza(size_t tot_object, size_t max_k){
            indexed_map.resize(tot_object);
            this->max_k = max_k;
            for (size_t i = 0; i < tot_object; i++){
        ...
        inline void heapifyDown(size_t i) {
            while (true) {
                size_t smallest = i;
                size_t left = 2 * i + 1;
                size_t right = 2 * i + 2;
        ...
        inline void heapifyUp(size_t i) {
            while (i > 0) {
                size_t parent = (i - 1) / 2;
        ...
        inline bool push(T & element, size_t index) {
        ...
        inline bool aggiorna( size_t index, T & element){
